...
const selectProduct = product.find(product => product.id === productId);
if (selectProduct) {
    document.getElementById("image").src = selectProduct.image
    const detailProductHTML = `
      <div class="detail" id="detail">
      <h3 class="d-title">${selectProduct.name}</h3>
      <h6 class="d-stok"> Stock : ${selectProduct.stock}</h6>
      <p class="d-dekripsi">${selectProduct.description}</p>
      <h6 class="d-harga">Harga : ${selectProduct.price}</h6>
      </div>`;
  document.getElementById("detail").innerHTML = detailProductHTML;
} else {
    document.body.innerHTML = "<p> Product tidak ditemukan!!</p>";
}
